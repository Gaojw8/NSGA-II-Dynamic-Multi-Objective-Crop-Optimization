function [indi]=Fix_cond(indi,Area)
indi = fix4(indi); %作物分散约束
indi = fix3(indi,Area); %三年豆类页约束
indi = fix1(indi);%若某块水浇地在某一年中种植了水稻，则将第一季设为种植的数量，第二季设置为0
                    %若不满足种植地限制约束，则将不满足约束的删去
indi = fix2(indi); %重茬约束
indi = fix5(indi,Area); %最小面积约束
indi = fix6(indi,Area); %最大面积约束

end

%% 三年豆类修复
function indi = fix3(indi,Area)
set = [1 2 3 4 5 17 18 19]; %豆类集合

for L=1:26 % 1-26遍历种粮食豆1-5
    con = 0;
    for year = 1:size(indi,2)
        ...
        if flag ==1 %找到了就跳过这块地
            ...
            con = con +1;
            if con==3
                indi{1,year}(L,randi([1,5])) = Area(L,2)*0.2;
                con=0;
            end
        end
    end
...
for L=27:50 %27-34 35-50遍历（只能在第一季）种蔬菜豆17-19
    con = 0;
    for year = 1:size(indi,2)
        flag = 0;
        for j=1:size(set,2) % 遍历每个豆
        ...
        if flag ==1 %找到了就跳过这块地
            ...
            con = con +1;
            if con==3
                indi{1,year}(L,randi([17,19])) = Area(L,2)*0.2;
                con=0;
            end
...
for L=79:82 % 79-82 一年两季内都可以种蔬菜豆 17-19
    con = 0;
    for year = 1:size(indi,2)
        flag = 0;
        for j=1:size(set,2) % 遍历每个豆
        ...
        if flag ==1 %找到了就跳过这块地
            ...
            con = con +1;
            if con==3
                indi{1,year}(L,randi([17,19])) = Area(L-54,2)*0.2;
                con=0;
            end
